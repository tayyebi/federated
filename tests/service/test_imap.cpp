...
TEST(imap_login_plain) {
    ...
    ErrorCode err = imap_handle_login(session, "testuser", "password", response, sizeof(response));
...
TEST(imap_list_mailboxes) {
    ...
    char response[1024];
    ErrorCode err = imap_handle_list(session, response, sizeof(response));
...
TEST(imap_select_mailbox) {
    ...
    char response[1024];
    ErrorCode err = imap_handle_select(session, "INBOX", response, sizeof(response));
...
TEST(imap_fetch_inbox) {
    ...
    char response[2048];
    ErrorCode err = imap_handle_fetch(session, "1", "BODY[]", response, sizeof(response));
